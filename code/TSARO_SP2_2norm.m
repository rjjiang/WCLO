function [z,y,u,v,w,v_opt,time,status] = TSARO_SP2_2norm(A,c,Q,b0)
%% Call the cplexmiqcp solver to solve the SP2 subproblem with p=2 in C&CG 
%% algorithm for TSARO problem. 
%
% The SP2 subproblm with p=2:
%     max        min     c'*z
% ||u||_2 <= 1   z\in R^m 
% s.t.  A*z <= Q*u+b0
%         z >= 0.
%
% Using KKT conditions, SP2 is equivalent to the following MIQCP:
%    max        c'*z
%  z,y,u,v,w
%  s.t.  A*z <= Q*u+b0,
%        A'*y <= c,
...
ub = [inf*ones(m,1);zeros(n,1);ones(r,1);inf*ones(m,1);inf*ones(n,1)];
